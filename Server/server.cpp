        ...
        for(;;)
        {
            ...
            if(socket->socketDescriptor() == Sessions[playerData.GameID - 1].Player1socketDescriptor)
            {
                ...
                if(Sessions[playerData.GameID - 1].GameStatus == "GameGoing")
                {
                    ...
                    else
                    {
                        ...
                        if (Sessions[playerData.GameID - 1].Player2.Status == "TurnGoing")
                        {
            ...
            else if (socket->socketDescriptor() == Sessions[playerData.GameID - 1].Player2socketDescriptor)
            {
                ...
                if(Sessions[playerData.GameID - 1].GameStatus == "GameGoing")
                {
                    ...
                    {
                        //Sessions[playerData.GameID - 1].TurnCount++;
                        if(Sessions[playerData.GameID - 1].Player1.Status == "TurnGoing")
                        {
...
void Server::SendToClient(Player &playerData)
{
    ...
    qDebug() << playerData.PlayerName << playerData.Status;
    for (int i = 0; i < playerData.CharacterList.size(); i++)
    {
        qDebug() << playerData.CharacterList[i].CharacterName;
        for(int j = 0; j < playerData.CharacterList[i].Effects.size(); j++)
        {
            qDebug() << playerData.CharacterList[i].Effects[j].EffectName
                     << playerData.CharacterList[i].Effects[j].Turns;
        }
    }
}
