    ...
    , ui(new Ui::QtGrandOrder)
{
    ...
    //Привязка картинок персонажей для выбора их активной целью + информация о персонаже + отображение хп мп
    ...
    for(int i = 0; i < 3; i++)
    {
        ...
        connect(CharacterLabels[i], SIGNAL(s_hoverEnter()), this ,SLOT(ShowCharacterInfo()));
        connect(CharacterLabels[i], SIGNAL(s_hoverLeave()), this ,SLOT(HideSkillInfo()));
    }
    for(int i = 3; i < 6; i++)
    {
...
void QtGrandOrder::GameSetUp() // Первичная настройка
{
    ...
    ActiveTarget = Opponent.CharacterList[0];
    CharacterLabels[3]->setStyleSheet("border-image: url(:CharacterData/CharacterData/" + Opponent.CharacterList[0].CharacterName + "_TargetImage.png) 0 0 0 0 stretch stretch;");
...
void QtGrandOrder::BattleUpdate() // Обновление игровой сцены
{
    ...
    for(int i = 0; i < Opponent.CharacterList.size(); i++)
    {
        ...
        //Отсчитывает количество оставшихся ходов активным эффектам и удаляет их
        if (CurrentPlayer.Status == "Waiting") //Тут надо прям ходы проверять, что сменились
        {
        ...
        {
            ActiveTarget = Opponent.CharacterList[i];
        }
    }
}
...
void QtGrandOrder::NextAliveLeft(int pos)
{
    int found = 0;
    int stunned = 0;
    CurrentPlayer.CharacterList[pos] = ActiveCharacter;
    for(int i = pos + 2; i >= pos; i--)
    {
        if(CurrentPlayer.CharacterList[i % 3].HP > 0)
        {
            for (int j = 0; j < CurrentPlayer.CharacterList[i % 3].Effects.size(); j++)
            {
                if(CurrentPlayer.CharacterList[i % 3].Effects[j].EffectName == "STUN")
                {
                    stunned = 1;
                    break;
                }
...
{
    int found = 0;
    Opponent.CharacterList[pos] = ActiveTarget;
    for(int i = pos + 1; i < pos + 3; i++)
    {
...
void QtGrandOrder::on_m_pB_BaseAttack_clicked()
{
    ...
    ActiveTarget = Opponent.CharacterList[index];
...
void QtGrandOrder::AllyLabelClicked()
{
    for (int i = 0; i < 3; i++)
    {
        if(qobject_cast<ClickableLabel*>(sender())->LabelCharacter.CharacterName == CurrentPlayer.CharacterList[i].CharacterName)
        {
            ActiveCharacter = CurrentPlayer.CharacterList[i];
            break;
        }
...
void QtGrandOrder::EnemyLabelClicked()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        if(qobject_cast<ClickableLabel*>(sender())->LabelCharacter.CharacterName == Opponent.CharacterList[i].CharacterName)
        {
            ActiveTarget = Opponent.CharacterList[i];
            break;
        }
...
{
    ui->m_Label_INFO->setText("Заплатите 60$ за механику скиллов");
    /*
    if (ActiveCharacter.MP >= ActiveCharacter.CharacterSkills[SkillsButtons.indexOf(sender())].MPCost)
    {
        ...
        case 1: //ENEMY
        {
            ...
            //Передача инфы о нажатом скилле игроку для правильного подсчета кд
            for (int i = 0; i < 3; i++)
            {
                if (ActiveCharacter.CharacterName == CurrentPlayer.CharacterList[i].CharacterName)
                {
...
void QtGrandOrder::ShowCharacterInfo()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        if (CurrentPlayer.CharacterList[i].CharacterName == qobject_cast<ClickableLabel*>(sender())->LabelCharacter.CharacterName)
        {
            InfoChar = CurrentPlayer.CharacterList[i];
            break;
        }
        if(Opponent.CharacterList[i].CharacterName == qobject_cast<ClickableLabel*>(sender())->LabelCharacter.CharacterName)
        {
            InfoChar = Opponent.CharacterList[i];
            break;
        }
    }

    QString info;
    info = "BaseMPRegen = " + QString::number(InfoChar.BaseMPRegen)
           + "\nDefence = " + QString::number(InfoChar.Defence)
           + "\nAttack = " + QString::number(InfoChar.BaseAttackDamage);
    for (int i = 0; i < InfoChar.Effects.size(); i++)
    {
        info += "\nEffect name = " + InfoChar.Effects[i].EffectName
                + "\nMagnitude = " + QString::number(InfoChar.Effects[i].Magnitude)
                + "\nTurns = " + QString::number(InfoChar.Effects[i].Turns);
    }
